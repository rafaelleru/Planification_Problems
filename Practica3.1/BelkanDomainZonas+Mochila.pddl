...
(define (domain BELKAN-DOMAIN)
	...
	(:action coger
		...
		:effect (and (not (manovacia)) (not (esta ?y ?x))(tengo ?x)(cogido ?x)))
	...
	(:action cargar
	  ...
	  :precondition (and (cogido ?x)(mochilaVacia))
	  :effect (and (not (mochilaVacia))(manovacia)(cargado ?x)))
	;ACC:sacar-mochila
	;LP:
	(:action sacar-mochila
	  :parameters (?x - objeto ?y - robot)
	  :precondition (and (not (mochilaVacia))(manovacia)(cargado ?x))
	  :effect (and (cogido ?x)(mochilaVacia)(not (manovacia))))
	...
	(:action dar
		...
		:effect (and (tiene ?y ?x) (manovacia) (not (tengo ?x)) (increase (entregados) 1))
	)
	...
	(:action mover
		...
		:precondition (and (not (ubicado ?y ?z))(ubicado ?x ?z)(camino ?x ?y)
						(tengo ?q)(and (clase ?y ?t)(or(permite-avanzar ?t ?q)(no-necesita-objeto ?t)))
