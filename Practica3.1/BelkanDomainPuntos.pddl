...
(define (domain BELKAN-DOMAIN)
	...
	(:action cargar
	  ...
	  :effect (and (not (mochilaVacia))(manovacia)(not (cogido ?x))(cargado ?x)))
	;ACC:sacar-mochila
	;LP:
	(:action sacar-mochila
	  :parameters (?x - objeto ?y - robot)
	  :precondition (and (manovacia)(cargado ?x))
	  :effect (and (cogido ?x)(mochilaVacia)(not (manovacia))(not (cargado ?x))))
	...
	(:action dar
		...
		:effect (and (tiene ?y ?x) (manovacia) (not (cogido ?x))(decrease (puntos) (suma-puntos ?x ?y))(increase (entregados) 1))
	)
